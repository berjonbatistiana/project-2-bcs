...
module.exports = {
  generateWord: (req, res) => {
    ...
    while (words.length < wordCount) {
      ...
      if (punctuation === "true") {
        ...
        const whichPunc = Math.floor(
          Math.random() * Math.floor(punctuations.length)
        );
  ...
  generateQuote: (req, res) => {
    try {
      const url =
        "https://opinionated-quotes-api.gigalixirapp.com/v1/quotes?rand=t&n=3&tags=short";
      axios.get(url).then((aRes) => {
        ...
        rawQuotes.forEach((el) => {
          ...
          arrQuotes.push(
            singleQuote.charAt(0).toUpperCase() + singleQuote.slice(1)
          );
        });
